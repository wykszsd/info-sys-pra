...
mockSchedulesDb.forEach(s => {
    ...
    if (!s.classroomBuilding || !s.classroomRoomNumber) {
        const classroom = MOCK_CLASSROOMS_SHORT.find(c => c.classroomId === s.classroomId);
        s.classroomBuilding = classroom?.building;
        s.classroomRoomNumber = classroom?.roomNumber;
    }
...
const createSchedule = async (payload: SchedulePayload): Promise<Schedule> => {
    if (USE_MOCK_API) {
        ...
        const newSchedule: Schedule = {
            ...
            classroomRoomNumber: classroom?.roomNumber,
...
const updateSchedule = async (id: number, payload: Partial<SchedulePayload>): Promise<Schedule> => {
    if (USE_MOCK_API) {
        ...
        if (payload.classroomId) { const cl = MOCK_CLASSROOMS_SHORT.find(cr => cr.classroomId === payload.classroomId); updatedSched.classroomBuilding = cl?.building; updatedSched.classroomRoomNumber = cl?.roomNumber; }
