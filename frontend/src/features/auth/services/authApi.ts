...
interface LoginResponse {
    ...
    user: { // 这是后端 /api/auth/login 返回的 user 对象结构
        userId: number;
        username: string;
        role: UserRole;
    };
        ...
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                ...
                if (foundUser) {
                    ...
                    const user = { userId: foundUser.userId, username: foundUser.username, role: foundUser.role as UserRole };
...
const fetchUserProfile = async (/* token 可以省略，因为 axiosInstance 会处理它 */): Promise<UserDetail> => {
    if (USE_MOCK_API) {
        ...
        const response = await axiosInstance.get<UserDetail>('/auth/me');
