...
export const fetchBuildingNamesList = createAsyncThunk<string[], void, { state: RootState, rejectValue: string }>(
    ...
    async (_, { getState, rejectWithValue }) => {
        if (getState().classrooms.buildings.length > 0 && !getState().classrooms.isLoading) {
            return getState().classrooms.buildings;
        }
        try {
...
const classroomSlice = createSlice({
    ...
    reducers: {
        ...
        clearClassroomsState: (state) => {
            Object.assign(state, initialState);
        }
    },
    extraReducers: (builder) => {
        builder
            .addCase(fetchBuildingNamesList.pending, (state) => { state.isLoading = true; /* Could be more granular */ })
            .addCase(fetchBuildingNamesList.fulfilled, (state, action) => {
                state.buildings = action.payload;
                // If this is the only initial load, set isLoading to false
                if (!state.query.startDate) state.isLoading = false;
            })
            .addCase(fetchBuildingNamesList.rejected, (state, action) => {
                state.isLoading = false; state.error = (state.error ? state.error + '; ' : '') + `教学楼: ${action.payload ?? '未知错误'}`;
            })
