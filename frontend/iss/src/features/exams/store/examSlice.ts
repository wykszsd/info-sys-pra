...
const examSlice = createSlice({
    ...
    reducers: {
        setCurrentExamSemesterId: (state, action: PayloadAction<number | null>) => {
            if (state.currentSemesterId !== action.payload) {
                ...
                state.myExams = []; state.error = null;
            }
        },
    ...
    extraReducers: (builder) => {
        builder
            .addCase(fetchStudentExams.pending, (state) => { state.isLoading = true; state.error = null; })
            .addCase(fetchStudentExams.fulfilled, (state, action) => { state.isLoading = false; state.myExams = action.payload; })
            .addCase(fetchStudentExams.rejected, (state, action) => { state.isLoading = false; state.error = action.payload ?? '获取考试安排未知错误'; });
    },
});
