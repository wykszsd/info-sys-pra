...
const AssignmentPage: React.FC = () => {
    ...
    useEffect(() => {
        if (currentSemesterIdForCourses && teacherCourses.length === 0 && !isLoadingCourses) {
            dispatch(fetchTeacherCoursesForAssignments(currentSemesterIdForCourses));
        }
    }, [dispatch, currentSemesterIdForCourses, teacherCourses.length, isLoadingCourses]);
    ...
    return (
        <Paper sx={{ p: { xs: 2, sm: 3 }, minHeight: '70vh' }}>
            ...
            {(!isLoadingCourses && !timetableCurrentSemester) && (
                <Alert severity="warning" sx={{ mt: 2 }}>请等待学期信息加载以选择目标课程。</Alert>
            )}

            {/* Render form when not loading courses AND semester info is available */}
            {!isLoadingCourses && timetableCurrentSemester && (
                <AssignmentForm
                    teacherCoursesForDropdown={teacherCourses}
                    onSubmitAssignment={handleSubmitAssignment}
                    isSubmitting={isSubmitting}
                    isLoadingCourses={isLoadingCourses} // Pass this to disable select while loading
                />
            )}
